...
foreach($userarr as $userval){
	...
	$chkuserarr[$userval['id']] = $userval['name'];
}

//form start

$form->add('label', 'label_project_id', 'project_id', 'For Project ');
$obj = $form->add('select', 'project_id', $data['project_id'],array("onchange"=>"getUser(this);"));

$rolearr = array(''=>'- Select Project -')+ $projectarr;
...
function getUser(obj){
	if($(obj).val() !=''){
		var url = '<?php echo $CNF->wwwroot?>projects/project/getAssignedUserOnProject';
		var opt ={
				"url" :url,
				"query":{
						"projectid":$(obj).val()
