...
?>
  ...
  <?php
		...
		$proname = $cntrlobj->form ['data']['projectname'];
		$srvname = $cntrlobj->form ['data']['servername'];
		...
		$selctedarr = $f3->get ( 'SESSION.search.deployment.exportsel' ) ? $f3->get ( 'SESSION.search.deployment.exportsel' ) : array ();
...
<div class="panel panel-warning">
	...
	<form name="nextstep" id="nextstep" role="form" action="<?php echo $CNF->wwwroot?>servers/deployment/deploy?step=2" method="post" class="Zebra_Form">
...
<?php if(!empty($selctedarr)){
	echo '<div class="nxtbutton" style="clear:both:margin:20px;text-align:center;width:100%;"><input type="submit" name="btnnext" id="btnnext" value="Next" class="submit btn btn-default"></div>';
}else{
echo '<div class="nxtbutton" style="clear:both:margin:20px;text-align:center;width:100%;display:none;"><input type="submit" name="btnnext" id="btnnext" value="Next" class="submit btn btn-default"></div>';
}
?>
...
foreach ( $format_data ['data'] as $record ) {
									if (array_key_exists ( 'NORECORD', $record )) {
										...
										$checked = '';
										if(!empty($selctedarr)){
											if(in_array($record ['id'],array_keys($selctedarr))){
												$checked ='checked';
											}
										}
										echo '<td><input type="checkbox" name="selectedexport[' . $record ['id'] . ']" value="' . $record ['id'] . '" val-name="' . $record ['projectname'] . " > " . $record ['rvname'] . " > " . $record ['issueid'] . '" class="chkexport" onchange="setExportSingle(this);" '.$checked.'/></td>';
...
function setValueAjax(sendContent) {
		...
		var url = '<?php echo $CNF->wwwroot?>servers/deployment/deploychangelogs';
		var opt ={
				"url" :url,
				"query":sendContent,
	...
	if(!$.isEmptyObject(data)){
		...
		try{
			...
			if($.isEmptyObject(sel.selected)){
				$listobj.html('<span class="label label-warning">No Changelog Selected For Deployment.</span> ');
				$('.nxtbutton').hide();
				}else{
					var count = 1;
